 ...
 // 	    setTimeout(function(){ $demoResult.val(result); }, 1000);
	...
	if(typeof JQ('#filter_gerentes').val() != 'undefined'){
	    JQ("#filter_gerentes").change(function (){
	    	let idGteV = parseInt(JQ(this).val());
	    	// console.log(idGteV);
	    	selectorAsesores(idGteV);
	  	});
	}

	// Imp. 24/08/21, Carlos, Cargar automaticamente los agentes de venta segun el gerente
	var idGteVentas = (JQ("#f_gerentes_id").val()!="")?parseInt(JQ("#f_gerentes_id").val()):0;
	if(idGteVentas>0){
