...
class  SasfeViewContactos extends JViewLegacy
{
	protected $items;
	...
	public function display($tpl = null)
	{
        ...
        protected function addToolBar()
        {
            ...
            $document->addStyleDeclaration('.icon-48-contactos {background-image: url(../media/com_sasfe/images/contactos.png);}');

           /*if($this->layout!="repetidos"){
                JToolBarHelper::title(JText::_('Contactos'), 'contacto');
            ...
            JToolBarHelper::title(JText::_('Contactos'), 'contacto');
            //Bloqueo para agentes de venta hasta arreglar el dato
            // if(in_array("8", $this->groups) || in_array("10", $this->groups) || in_array("11", $this->groups) || in_array("19", $this->groups)){
                if(!in_array("19", $this->groups)){
                    JToolBarHelper::addNew('contacto.addnsu');
                }
            // }
            JToolBarHelper::cancel('contactos.cancel');

            //submenu
            JSubMenuHelper::addEntry('Escritorio', 'index.php?option=com_sasfe', false);
            if(in_array("8", $this->groups) || in_array("10", $this->groups) || in_array("17", $this->groups) || in_array("18", $this->groups) || in_array("11", $this->groups) || in_array("19", $this->groups) || in_array("20", $this->groups)){

                if(in_array("8", $this->groups) || in_array("10", $this->groups) || in_array("19", $this->groups) || in_array("20", $this->groups)){
                    if(in_array("19", $this->groups)){
                        JSubMenuHelper::addEntry('Contactos', 'index.php?option=com_sasfe&view=contactos', true);
                    }else{
            ...
            if(!in_array("20", $this->groups) && !in_array("18", $this->groups) && !in_array("17", $this->groups) && !in_array("11", $this->groups)){
        ...
        protected function setDocument()
        {
            ...
            $document = JFactory::getDocument();
            $document->setTitle(JText::_('Contactos'));
            ...
            $document->addScript(JURI::root().'administrator/components/com_sasfe/views/contactos/submitbutton.js');
        }
}
