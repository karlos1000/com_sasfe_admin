...
$agtVentasId = "";
$datosAgtVentas = array();
$colAsesores = array();
$usrActivo = 0;
if(in_array("18", $this->groups)){
    $agtVentasId = $this->user->id;
    $datosAgtVentas = SasfehpHelper::obtUsuarioDatosCatalogoPorIdUsrJoomla($agtVentasId);
    // //obtener los asesores por el id del gerente de ventas     
    // $colAsesores = SasfehpHelper::obtColAsesoresAgtVentaXIdGteVentas($agtVentasId);
    // //Obtener cuantos creditos tiene el gerente de ventas
    $colCreditos = SasfehpHelper::checkCreditoPorUsuarioIdSMS($agtVentasId);    
    $creditosDisp = 0;
    if(count($colCreditos)>0){
      $creditosDisp = $colCreditos->creditos;
      $usrActivo = $colCreditos->activo;
    }
    // echo "idAgt: ".$agtVentasId.'<br/>';
    // echo "<pre>";
    // print_r($colCreditos);
    // echo "</pre>";
}else{
...
$colMensajes = SasfehpHelper::obtPromocionesSMS();
// echo "<pre>";
// print_r($colMensajes);
// echo "</pre>";
...
<form class="form-validate form-horizontal" action="<?php echo JRoute::_('index.php?option=com_sasfe&view=smsenviospromociones'); ?>" method="post" name="adminForm" id="adminForm">
    <div class="alert alert-info">
      <strong>Nota:</strong>      
      <div>1.- El estatus y el rango de fechas de captura son opcionales, si elige alguna presionar despues el bot&oacute;n de buscar.</div>
    ...
    <div class="alert alert-warning">      
      <div>Estas inactivo para enviar mensajes, para resolverlo contacta con tu gerente.</div>
    </div>
    <?php } ?>
    <div>        
        ...
        <div class="col50">
            <div class="control-group">    
                ...
                <div class="controls">                    
                        <?php
                            if(count($datosAgtVentas)>0){
                               echo '<option value="' . $datosAgtVentas[0]->idDato . '">' . $datosAgtVentas[0]->nombre . '</option>';
                               echo '<input type="hidden" id="agtventas" name="agtventas" value="'.$datosAgtVentas[0]->idDato.'" />';
                            }else{
                                ?>
                                ...
                                <?php                                
                                // echo '<input type="hidden" id="agtventas" name="agtventas" value="" />';
                            }                        
                        ?>                    
                </div>
            </div>
            ...
            <div class="controls">
                <select name="preconf_msn" id="preconf_msn" class="required">
                    ...
                    foreach ($colMensajes as $itemMsg) {
                        echo '<option idMsg="'.$itemMsg->idPromo.'"  value="' . $itemMsg->texto . '" >' . $itemMsg->titulo . '</option>';                        
                    }
                    ?>
                </select>
            </div>
    ...
    <div>        
        ...
        <!-- <input type="hidden" name="usuarioIdAgtVenta" value="?php echo $agtVentasId; ?>" />         -->
