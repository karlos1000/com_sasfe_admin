...
class SasfeControllerSmsmensajes extends JControllerForm {
    ...
    function delete(){        
        ...
        $idsSmsMensajes = JRequest::getVar('cid', array(), '', 'array');                
        // Sanitize the input
        JArrayHelper::toInteger($idsSmsMensajes);
        
        // $pathImgs = JPATH_SITE.'/media/com_sasfe/upload_files';  
        $model = JModelLegacy::getInstance('smsmensajes', 'SasfeModel'); 
        $result = $model->removerPorIdMensaje($idsSmsMensajes);             
       ...
       if(count($result['resultDel'])>0){                                 
            ...
            $this->setRedirect('index.php?option=com_sasfe&view=smsmensajes', $msn);
        }
        else{
            ...
            $this->setRedirect('index.php?option=com_sasfe&view=smsmensajes', $text);
        }
