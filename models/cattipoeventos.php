...
class SasfeModelCattipoeventos extends JModelList{
        ...
        protected function populateState($ordering = null, $direction = null) {
            ...
            parent::populateState('idTipoEvento', 'asc');
        }
            ...
            if (!isset($this->items))
            {
                ...
                if($search){                                        
                   ...
                   $searches = array();                                         
                    ...
                    $searches[] = "a.idTipoEvento LIKE '%$search%' "; //Buscar por id del fraccionamiento
                    $searches[] = "a.tipoEvento LIKE '%$search%' "; //Buscar por nombre de fraccionamiento                                       
                ...
                $query = "
                        SELECT a.*
                        FROM #__sasfe_tipo_eventos as a 
                        $searchQuery 
                      ";                                       
                // echo $query;
        ...
        public function removerPorIdEvento($idsEventos){
            $db = JFactory::getDbo();                                                                                        
            $noBorrados = array();
            $result = array();
            ...
            foreach($idsEventos as $id):
                ...
                $queryTipo = "SELECT * FROM #__sasfe_movimientosprospecto WHERE tipoEventoId IN ($id)";
                $db->setQuery($queryTipo);
                $rowsTipoEvento = $db->loadObjectList();                                                                  
                if(count($rowsTipoEvento)>0){
                    $noBorrados[] = $id;
                }else{
                    $query = "DELETE FROM #__sasfe_tipo_eventos WHERE idTipoEvento=$id ";
                    $db->setQuery($query);
                    $db->query();
                    $result[] = $id;
                }
