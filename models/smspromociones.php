...
class SasfeModelSmspromociones extends JModelList{
        ...
        protected function populateState($ordering = null, $direction = null) {
            ...
            parent::populateState('idPromo', 'asc');
        }
            ...
            if (!isset($this->items))
            {
                ...
                if($search){                                        
                   ...
                   $searches = array();                                         
                    ...
                    $searches[] = "a.idPromo LIKE '%$search%' "; //Buscar por id del mensaje
                ...
                $query = "
                        SELECT a.*
                        FROM #__sasfe_sms_promociones as a 
                        $searchQuery 
                      ";                                       
                // echo $query;
        ...
        public function removerPoridPromo($idsPromos){
            $db = JFactory::getDbo();                                                                                        
            $noBorrados = array();
            $result = array();
            
            foreach($idsPromos as $id):
                //Antes de remover verificar que ninguno de este tipo de eventos este en uso
                // $queryTipo = "SELECT * FROM #__sasfe_sms_promociones WHERE idPromo IN ($id)";
                ...
                //     $noBorrados[] = $id;
                // }else{
                    $query = "DELETE FROM #__sasfe_sms_promociones WHERE idPromo=$id ";
                    $db->setQuery($query);
                    $db->query();
                    $result[] = $id;
                // }
